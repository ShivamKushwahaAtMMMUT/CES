...
if ( isset( $_POST[ 'update_member' ] ) ) {
   //Retreive form data
   $member_id = trim( htmlspecialchars( $_POST[ 'member_id' ] ) );
   ...
   if ( !empty( $_FILES[ 'image' ][ 'tmp_name' ] ) ) {
      $query = "UPDATE members SET name=?, image=?, year=?, designation=?, linkedin=?, facebook=?, email=? WHERE member_id=?";
      $stmt = $conn->prepare( $query );
      $stmt->bind_param( 'ssissssi', $member_name, $image, $year, $designation, $linkedin, $facebook, $email, $member_id );
      $stmt->execute();
   } else {
      $query = "UPDATE members SET name=?, year=?, designation=?, linkedin=?, facebook=?, email=? WHERE member_id=?";
      $stmt = $conn->prepare( $query );
      $stmt->bind_param( 'sissssi', $member_name, $year, $designation, $linkedin, $facebook, $email, $member_id );
      $stmt->execute();
   }
   $conn->close();
}
